...
/*
 * AI analysis orchestration: enhance an uploaded blueprint image, run the local
 * vision model for OCR + structure extraction, and normalize to BlueprintData.
 * Behind a service so the route is a thin adapter and the pipeline can move to a
 * remote vision backend later without UI changes.
 */
...
/**
 * Ollama in this setup accepts only ONE image. We build one light contact sheet
 * (full enhanced blueprint + one zoom crop) — intentionally lighter than a
 * full + 3-crop version, because qwen2.5vl runs on CPU and a bigger image
 * caused fetch failures.
 */
async function createSingleAnalysisImage(imageBase64: string): Promise<string> {
  ...
  for (let i = 0; i < cropCount; i++) {
    ...
    cropPanels.push(await enhancePanel(cropRaw, targetWidth));
  }
  ...
  for (let i = 0; i < labeledPanels.length; i++) {
    composite.push({ input: labeledPanels[i], left: 0, top: currentTop });
...
export async function analyzeImage(imageBase64: string): Promise<BlueprintData> {
  ...
  return normalizeBlueprintData(parsed);
}
